...
*/
int InitApp(
	int bus_num		//设备总线号，从设备管理器查看（相同背板的机箱的总线号是一直固定的）
);
...
/*写19eg寄存器*/
ULONG WriteRegTo19eg(
	int hDev,							//类型：输入，参数输入值固定，即hDevice
	ULONG RegisterOffset_Api,			//类型：输入，寄存器号，输入值：0~255（10进制）
	ULONG RegisterData_Api				//类型：输入，寄存器值，输入值：0~0xFFFFFFFF
	);

ULONG ReadRegFrom19eg(
	int hDev,
	ULONG RegisterOffset_Api,
	ULONG& RegisterData_Api
	);

/*写115寄存器*/
ULONG WriteRegToFpga(
	int hDev,							//类型：输入，参数输入值固定，即hDevice
...
ULONG StratTransferDataFromFpga(
	int hDev,						//类型：输入，参数输入值固定，即hDevice
	UCHAR Channel_Api,				//类型：输入，通道选择，输入值：1或2
	ULONG Sample_Api,				//类型：输入，单次采样量选择，输入值：SAMPLE_SIZE_MAX、SAMPLE_SIZE_4MB、SAMPLE_SIZE_1MB、SAMPLE_SIZE_256KB、SAMPLE_SIZE_64KB （TRANSFER_MODE_BLOCK模式下固定为SAMPLE_SIZE_4MB）
	ULONG Mode_Api,					//类型：输入，传输模式选择，输入值：TRANSFER_MODE_FOREVER、TRANSFER_MODE_LENGTH、TRANSFER_MODE_BLOCK
	ULONG Length_Api				//类型：输入，传输长度，输入值：NULL（TRANSFER_MODE_FOREVER）、0~0xC0000000KB 即 最大3TB（TRANSFER_MODE_LENGTH）、0~0x200000KB 即 最大2GB（TRANSFER_MODE_BLOCK）
);
...
/*结束数据流传输*/
ULONG StopTransferDatafromFpga(
	int hDev,					//类型：输入，参数输入值固定，即hDevice
	UCHAR Channel_Api			//类型：输入，通道选择，输入值：1或2
);
...
unsigned int ReadDataFromFpga(
	...
	UCHAR& Channel_Api,					//类型：输出，输出值：当前数据块的来源（数值：1:数据来源于1号、2：数据来源于2号、0：没有读取到FPGA的数据）
	ULONG& Data_Type,
...
ULONG StartTransferDataToFpga(
	int hDev,						//类型：输入，参数输入值固定，即hDevice
	ULONG DDRBaseAddr_Api			//类型：输入，DDR或BRAM基址，如果是DDR则输入值为1字节单位，如果BRAM则输入值为4字节单位，下发4240后，4240会*4
);
/*
函数返回值：
...
ULONG CreateFileInStorage(
	int hDev,							   //类型：输入，参数输入值固定，即hDevice
	char* FileName_Api,                    //类型：输入，文件名（最长31字节），输入一个32字节数组（除了文件名外，其它空余位请置0，最后一个字节必须为0，详见示例）
...
ULONG StopFillFile(
	int hDev,							 //类型：输入，参数输入值固定，即hDevice
	char* FileName_Api,                  //类型：输入，文件名（最长31字节），输入一个32字节数组（除了文件名外，其它空余位请置0，最后一个字节必须为0）
	ULONG WaitTime                       //类型：输入，等待PowerPC返回时间（单位：ms），如果<=1000ms,则自动设定1000ms
);
/*函数返回值
TIME_OUT_FUNC                       超时
...
ULONG StartFileTransfer(int hDev,
	char* FileName_Api,
	char* buf,
	ULONGLONG& nRead,
	ULONGLONG FileOffset_Api,
	ULONGLONG FileGetSize_Api,
	ULONG WaitTime);
/*函数返回值
