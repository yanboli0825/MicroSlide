...
{
    //写config.ini
    QString clarity_temp = ui->lineEdit_clarity->text().remove(QRegularExpression("\\s+")); //QRegularExpression是Qt里的一个类，这里用来创建了一个正则表达式匹配文本里的所有空白（"\s+"）字符[若为\s则只匹配一个空白字符]，然后用remove()移除
    clarity_temp.remove(QRegularExpression("[,，]$"));//正则表达式匹配字符串末尾的英文和中文逗号。[]是字符集，$是锚点，表示末尾
    ...
    box->exec();// 这行代码显示消息框，并进入一个模态事件循环，直到消息框被关闭。exec 方法将使消息框成为模态对话框，这意味着在用户关闭消息框之前，他们不能与其他窗口交互
    emit signal_syscfgChanged(clarity_temp.toInt(), similarity_temp.toInt(), area_temp.toInt(), filePath_temp, db_addr_temp.toStdString(), port_temp.toInt());
